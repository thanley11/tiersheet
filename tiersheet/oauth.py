...
qbs = yahoo.get('http://fantasysports.yahooapis.com/fantasy/v2/users;use_login=1/games;game_keys=nfl/players;position=QB?format=json').json()
...
myResult = qbs["fantasy_content"]["users"]["0"]["user"][1]["games"]["0"]["game"][1]["players"]
for x in myResult:
    print x, 'corresponds to', myResult[x]["player"][0][2]["name"]["full"]
